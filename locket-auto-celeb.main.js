(function() {
    ...
    /**
     * Tiêm CSS (ĐÃ CẬP NHẬT - Xóa style cho Key Wall)
     */
    function injectNewStyles() {
        ...
        style.textContent = `
            ...
            /* ===== Giao diện Key Wall (ĐÃ BỊ XÓA) ===== */
            /* ===== Trạng thái Khóa (Locked) (ĐÃ BỊ XÓA) ===== */
    ...
    /**
     * Tạo Bảng điều khiển CHÍNH (ĐÃ CẬP NHẬT - Xóa Key Wall)
     */
    function createMainControlUI() {
        ...
        // 2. MỚI: Key Wall (ĐÃ BỊ XÓA)
        // Phần tạo HTML cho Key Wall đã bị loại bỏ
    ...
    /**
     * Cài đặt TOÀN BỘ điều khiển UI (ĐÃ CẬP NHẬT - Xóa logic Key Wall)
     */
    function setupMainUIControls() {
        ...
        // --- MỚI: Logic cho Key Wall (ĐÃ BỊ XÓA) ---
        // Toàn bộ logic `validateKey` và listener cho `btn-submit-key` đã bị xóa
        ...
        if (plusBtn && minusBtn && toggleInput && timerUI) {
            ...
            minusBtn.addEventListener('click', (event) => {
                event.stopPropagation();
                if (activeTimerId) return;
                if (currentTimerConfig.minutes > 2) {
                    currentTimerConfig.minutes -= 2;
                    log(`Giảm thời gian hẹn giờ xuống: ${currentTimerConfig.minutes} phút.`, 'timer');
                } else {
                    currentTimerConfig.minutes = 3;
                    log(`Thời gian hẹn giờ tối thiểu là 3 phút.`, 'timer');
                }
                saveTimerConfig();
                updateTimerUI();
            });
...
function waitForElementById(elementId, timeout = 180000, interval = 500) {
    ...
    async function processNextCeleb(celebIds, totalCount) {
        ...
        if (startButton) {
            ...
            await sleep(5000); // Chờ 5s để log bắt đầu chạy
    ...
    // --- Main Execution (ĐÃ CẬP NHẬT - Xóa check Key) ---
    (function main() {
        ...
        // --- MỚI: Kiểm tra Key (ĐÃ BỊ XÓA) ---
        // Toàn bộ logic kiểm tra key và gán class 'locked' đã bị xóa.
        // Script sẽ luôn ở trạng thái "mở khóa".
        const container = document.getElementById('auto-celeb-main-container');
        if(container) {
             container.classList.remove('locked'); // Xóa class locked nếu có (dù logic gán đã bị xóa)
        }
        // --- Hết: Kiểm tra Key ---
        ...
        if (window.location.href === CONFIG.TARGET_PAGE) {
            ...
            if (!controlButton) {
                // Chỉ báo lỗi nếu không tìm thấy nút
                console.error('[Auto Locket Celeb] ➡️ Không tìm thấy control button trên trang target.');
                return;
            }
            ...
            // Chạy logic tự động (không cần check key)
            runCelebLogic();
        ...
        async function runCelebLogic() {
            try {
                ...
                if (needsTimerRestart) {
                    log('PHÁT HIỆN CỜ RESTART. Tự động bắt đầu sau 5 giây...', 'timer');
                    localStorage.removeItem(CONFIG.TIMER_RESTART_KEY);
